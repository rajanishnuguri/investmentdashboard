...
const api = {
  ...
  connect: (u,b)=>fetch(`/api/${u}/${b}/connect`,{method:"POST"}).then(r=>r.json().then(j=>({ok:r.ok,...j}))),
...
function Holdings({allHoldings, refreshedAt}){
    ...
    <div className="space-y-4">
      {refreshedAt && (
        <div style={{color:C.muted,fontSize:12}}>
          Data as of {timeAgo(refreshedAt)} · values from broker MCP
        </div>
      )}
...
/* ─── Data Room ─── */
function DataRoom({user, brokers, onCall}){
...
/* ─── Family Dashboard ─── */
function FamilyDashboard({usersState}){
  ...
  const perUser = useMemo(()=>{
    return USERS.map(([uid, name])=>{
      ...
      return { uid, name, holdings, total };
    });
  },[usersState]);
  ...
  }),{invested:0,current:0,pnl:0}),[perUser]);
    ...
    <div className="space-y-5">
      ...
      <div className="grid sm:grid-cols-3 gap-3">
        ...
        <Panel className="p-4 sm:col-span-1" style={{background:"linear-gradient(135deg, rgba(0,229,168,0.14), rgba(79,168,255,0.08))"}}>
          ...
          <div className="font-mono" style={{color:C.text,fontSize:28,fontWeight:700,lineHeight:1}}>{familyTotal.current?cr(familyTotal.current):"—"}</div>
          <div className="mt-1 font-mono" style={{color:familyTotal.pnl>=0?C.pos:C.neg,fontSize:12}}>
            {familyTotal.pnl?cr(familyTotal.pnl):"—"} {familyTotal.invested?`(${pct((familyTotal.pnl/familyTotal.invested)*100)})`:""}
          </div>
        </Panel>
        {/* Per person */}
        {perUser.map(u=>(
          <Panel key={u.uid} className="p-4">
            ...
            <div className="mt-1 font-mono" style={{color:u.total.pnl>=0?C.pos:C.neg,fontSize:12}}>
              {u.total.pnl?cr(u.total.pnl):"—"} {u.total.invested?`(${pct((u.total.pnl/u.total.invested)*100)})`:""}
            </div>
            <div style={{color:C.muted,fontSize:11,marginTop:4}}>{u.holdings.length} holdings</div>
          </Panel>
        ))}
      </div>
      ...
      {tab==="overview" && <Overview total={familyTotal} allHoldings={allHoldings} a={{currentAge:0}} proj={{rows:[],fiYear:null,todayFI:0}} go={null}/>}
      {tab==="holdings" && <Holdings allHoldings={allHoldings} refreshedAt={null}/>}
...
function UserDashboard({uid, uLabel, brokers, onConnect, onLoad, onCall}){
  ...
  const total = useMemo(()=>{
    let invested=0,current=0,pnl=0,dividends=0;
    for(const h of allHoldings){
      invested+=h.invested||0; current+=h.current||0;
      pnl+=h.absoluteReturn||h.pnl||0;
      dividends+=h.dividendEarned||0;
    }
    return {invested,current,pnl,dividends,totalReturn:pnl+dividends};
  },[allHoldings]);
  ...
  return (
    <div className="space-y-5">
      ...
      {tab==="holdings"    && <Holdings allHoldings={allHoldings} refreshedAt={refreshedAt}/>}
      {tab==="allocation"  && <Allocation allHoldings={allHoldings}/>}
      {tab==="trajectory"  && <Trajectory a={a} setA={setA} proj={proj}/>}
      {tab==="data"        && <DataRoom user={uid} brokers={brokers||{}} onCall={onCall}/>}
    </div>
  );
}
...
function App(){
  ...
  const [users, setUsers] = useState({ rajanish:{brokers:{}}, aswini:{brokers:{}} });
  ...
  function patchBroker(user, broker, v){
    setUsers(prev=>({
      ...
      [user]:{
        ...
        brokers:{
          ...(prev[user]?.brokers||{}),
          [broker]:{...(prev[user]?.brokers?.[broker]||{}),...v}
        }
      }
    }));
  }
  ...
  async function connect(user, broker){
    ...
    try{
      ...
      patchBroker(user,broker,{connected:true,tools:r.tools,loginUrl:r.loginUrl,loading:false});
      if(r.loginUrl) window.open(r.loginUrl,"_blank","noopener");
    }catch(e){ patchBroker(user,broker,{loading:false,error:String(e.message||e)}); }
  }
  ...
  async function load(user, broker){
    ...
    try{
      ...
      if(r.status===401 && r.loginUrl){ window.open(r.loginUrl,"_blank","noopener");
        patchBroker(user,broker,{loading:false,error:"Login needed — finish in the new tab, then Load again."}); return; }
      if(!r.ok) throw new Error(r.error||"load failed");
      const savedAt = r.savedAt || new Date().toISOString();
      patchBroker(user,broker,{authed:true,holdings:r.holdings||[],tools:r.tools||(users[user]?.brokers?.[broker]?.tools||[]),loading:false,fromCache:!!r.fromCache,cachedAt:savedAt});
    }catch(e){ patchBroker(user,broker,{loading:false,error:String(e.message||e)}); }
  }
  ...
  const USER_TABS = [
    ["rajanish","Rajanish",C.rajanish],
    ["aswini","Aswini",C.aswini],
    ["family","Family","linear-gradient(90deg,"+C.rajanish+","+C.aswini+")"],
  ];
  ...
  return (
    <div style={{background:C.bg,color:C.text,minHeight:"100vh",position:"relative"}}>
      ...
      <div className="relative max-w-5xl mx-auto px-4 md:px-6 py-5" style={{zIndex:1}}>
        ...
        {userTab==="family" && (
          <FamilyDashboard usersState={users}/>
        )}
